...
const OrdersPage = ({ order, products, customers }: OrdersProps) => {
	...
	const handleTaxesChange = (e: React.ChangeEvent<HTMLInputElement>) => {
		const salesTax = (parseInt(e.target.value) / 100) * currentOrder.products.reduce((acc, item) => acc + (item.sellPrice * item.quantity), 0);
	...
	useEffect(() => {
		...
		setCurrentOrder(prevState => Object.assign({}, prevState, {
			total: finalTotal,
			salesTax: salesTax
		}));
		...
		},
		{
			...
			content: (orderItem) => (
				<InputNumber
					...
					handleChange={(e) => handleProductPropertyChange(orderItem, 'quantity', parseInt(e.currentTarget.value))}
				/>
			),
		},
		{
			...
			content: (orderItem) => (
				<InputNumber
					...
					handleChange={(e) => handleProductPropertyChange(orderItem, 'buyPrice', parseInt(e.currentTarget.value))}
				/>
			),
		},
		{
			...
			content: (orderItem) => (
				<InputNumber
					...
					handleChange={(e) => handleProductPropertyChange(orderItem, 'sellPrice', parseInt(e.currentTarget.value))}
				/>
			),
		},
		{
		...
		<div className='orders-page'>
			<div className='admin-order-page-header'>
				...
				<div className='flex flex-col'>
					...
					{currentOrder.status == OrderStatus.WaitingCustomerApproval && (
						<button onClick={() => setOrderStatus(OrderStatus.Placed, true)} disabled={!currentOrder.customerId}>Place order</button>
					)}

					{currentOrder.status == OrderStatus.Placed && (
						<button onClick={() => setOrderStatus(OrderStatus.Processing, true)} disabled={!currentOrder.customerId}>Start Processing Order</button>
					)}

					{currentOrder.status >= OrderStatus.Placed && (
						<button onClick={handleSubmitInvoice} disabled={!currentOrder.customerId}>Submit Invoice to customer</button>
					)}

					{currentOrder.status == OrderStatus.Processing && (
						<button  onClick={() => setOrderStatus(OrderStatus.Shipped, true)} disabled={!currentOrder.customerId}>Order has been shipped</button>
					)}

					{currentOrder.status == OrderStatus.Shipped && (
						<button  onClick={() => setOrderStatus(OrderStatus.Delivered, true)} disabled={!currentOrder.customerId}>Order has been delivered</button>
					)}
			...
			<InputNumber
				label='Total' 
				value={currentOrder.total.toString()} 
				disabled={true} 
			/>
