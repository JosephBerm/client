...
const mockMatrix: PermissionMatrixEntry[] = [
	createMockMatrixEntry({
		...
		roleAccess: { 0: false, 100: true, 200: true, 9999999: true },
	}),
	createMockMatrixEntry({
		resource: 'quotes',
		action: 'create',
	...
	createMockMatrixEntry({
		...
		roleAccess: { 0: false, 100: false, 200: true, 9999999: true },
	}),
	createMockMatrixEntry({
		resource: 'orders',
		action: 'create',
...
describe('PermissionMatrix Component', () => {
	...
	describe('Editable Mode (canEdit = true)', () => {
		it('should call onPermissionToggle when clicking permission button', async () => {
			...
			const buttons = screen.getAllByRole('button')
			const toggleButton = buttons.find((btn) => btn.title?.includes('access') || btn.closest('td'))
		...
		it('should pass correct parameters when toggling', async () => {
			...
			const toggleButtons = screen
				.getAllByRole('button')
				.filter((btn) => btn.title?.includes('access') && !btn.title?.includes('Admin'))
		...
		it('should not allow toggling Admin role permissions', async () => {
			...
			const adminButtons = screen.getAllByRole('button').filter((btn) => btn.title?.includes('Admin always'))
	...
	describe('Read-Only Mode (canEdit = false)', () => {
		it('should not call onPermissionToggle when canEdit is false', async () => {
			...
			const buttons = screen.getAllByRole('button').filter((btn) => btn.title?.includes('access'))
