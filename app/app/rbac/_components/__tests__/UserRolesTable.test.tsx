...
function createMockUsersData(count: number = 5): PagedResult<UserWithRole> {
  const users = Array.from({ length: count }, (_, i) =>
    createMockUser({
      ...
      fullName: `User ${i + 1}`,
      role: [AccountRole.Customer, AccountRole.SalesRep, AccountRole.SalesManager, AccountRole.Admin][i % 4],
...
describe('UserRolesTable Component', () => {
  ...
  describe('Rendering', () => {
    ...
    it('should display role names', () => {
      ...
      const roleElements = screen.getAllByText(/Sales Representative|Customer|Sales Manager|Administrator|Fulfillment Coordinator/)
  ...
  describe('Loading State', () => {
    it('should show loading indicator when isLoading is true', () => {
      render(<UserRolesTable {...defaultProps} isLoading={true} users={null} />)
  ...
  describe('Error State', () => {
    it('should display error message when error is provided', () => {
      render(<UserRolesTable {...defaultProps} error="Failed to load users" />)

      expect(screen.getByText(/Failed to load users/i)).toBeInTheDocument()
    })

    it('should show error icon with message', () => {
      render(<UserRolesTable {...defaultProps} error="Error occurred" />)
  ...
  describe('Empty State', () => {
    it('should show empty message when no users', () => {
      ...
      render(<UserRolesTable {...defaultProps} users={emptyUsers} />)

      expect(screen.getByText(/no users/i)).toBeInTheDocument()
    })

    it('should show empty state when users is null', () => {
      render(<UserRolesTable {...defaultProps} users={null} isLoading={false} />)
  ...
  describe('Bulk Update', () => {
    ...
    it('should call onBulkUpdate when bulk update button is clicked', async () => {
      ...
      render(<UserRolesTable {...defaultProps} onBulkUpdate={onBulkUpdate} />)
  ...
  describe('Table Content', () => {
    ...
    it('should show active status with appropriate styling', () => {
      ...
      render(<UserRolesTable {...defaultProps} users={singleUser} />)
    ...
    it('should handle user with no last login', () => {
      ...
      render(<UserRolesTable {...defaultProps} users={userNoLogin} />)
    ...
    it('should format last login date correctly', () => {
      ...
      render(<UserRolesTable {...defaultProps} users={userWithLogin} />)
  ...
  describe('Edge Cases', () => {
    it('should handle very long user names', () => {
      ...
      render(<UserRolesTable {...defaultProps} users={longNameUser} />)
    ...
    it('should handle very long email addresses', () => {
      const longEmailUser = {
        ...defaultProps.users,
        data: [createMockUser({ email: 'verylongemailaddressthatmightoverflow@verylongdomainname.example.com' })],
      } as PagedResult<UserWithRole>

      render(<UserRolesTable {...defaultProps} users={longEmailUser} />)
    ...
    it('should handle many users', () => {
      ...
      render(<UserRolesTable {...defaultProps} users={manyUsers} />)
