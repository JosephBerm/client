...
export default function RoleFormModal({
	...
	const levelConflict = existingRoles.find((r) => r.level === safeWatchedLevel && r.id !== role?.id)
			...
			<form
				...
				<fieldset className='space-y-4'>
							...
							{LEVEL_PRESETS.map((preset) => {
								...
								return (
									<Button
										...
										size='sm'
										className={`
											flex flex-col items-center gap-1 p-2 sm:p-3 rounded-lg border-2 transition-all h-auto
											${
												isSelected
										...
										`}
										contentDrivenHeight>
										<span className={`${isSelected ? 'text-primary' : 'text-base-content/60'}`}>
											{preset.icon}
										</span>
					...
					{!isEditing && safeWatchedLevel >= DEFAULT_ROLE_THRESHOLDS.adminThreshold && (
						<div className='rounded-lg bg-warning/10 border border-warning/20 p-3'>
							<div className='flex items-start gap-2'>
								...
								<p className='text-sm text-warning/80'>
									Level {safeWatchedLevel}+ grants administrative access. Users with this role will
									have elevated privileges. Ensure this is intended.
								</p>
							</div>
						</div>
					)}
					...
					{existingRoles.length > 0 && (
						<div className='bg-base-200/50 rounded-lg p-3'>
							<p className='text-xs font-medium text-base-content/60 mb-2'>Position in Role Hierarchy</p>
				...
				<div className='border border-base-300 rounded-lg overflow-hidden'>
					<Button
						...
						variant='ghost'
						className='w-full flex items-center justify-between p-3 hover:bg-base-200/50 transition-colors h-auto'
						rightIcon={
							showAdvanced ? (
								...
								<div className='grid grid-cols-2 sm:grid-cols-4 gap-2 mt-2'>
									{BADGE_VARIANTS.map((variant) => (
										<Button
											...
											variant={watchedBadgeVariant === variant.value ? 'primary' : 'outline'}
											size='sm'
											className={`
												flex flex-col items-center gap-1 p-2 rounded-lg border transition-all h-auto
												${watchedBadgeVariant === variant.value ? 'border-primary bg-primary/10' : 'border-base-300 hover:border-primary/50'}
											`}
											contentDrivenHeight>
											<Badge
