...
function createMockMatrixEntry(overrides: Partial<PermissionMatrixEntry> = {}): PermissionMatrixEntry {
	return {
		...
		description: 'Read all quotes',
		roleAccess: {
			[RoleLevels.Admin]: true,
			[RoleLevels.FulfillmentCoordinator]: true,
			[RoleLevels.Customer]: false,
		},
		...overrides,
	}
}
...
function createMockOverview(overrides: Partial<RBACOverview> = {}): RBACOverview {
	return {
		roles: [
			createMockRole({ id: 1, role: RoleLevels.Admin, name: 'admin', displayName: 'Administrator', level: 500 }),
			createMockRole({
				id: 2,
			...
			createMockRole({ id: 4, role: RoleLevels.Customer, name: 'customer', displayName: 'Customer', level: 100 }),
		],
		permissions: [
			...
			createMockPermission({
				...
				action: 'create',
				context: null,
				permissionString: 'quotes:create',
			}),
			createMockPermission({
				id: 3,
				resource: 'orders',
				action: 'read',
				context: 'own',
				permissionString: 'orders:read:own',
			}),
		],
		matrix: [
		...
		userStats: {
			...
			countByRole: {
				[RoleLevels.Admin]: 5,
				[RoleLevels.SalesManager]: 10,
				[RoleLevels.SalesRep]: 50,
				[RoleLevels.Customer]: 85,
			},
		},
		...overrides,
	}
...
describe('useRBACManagement Hook - RBAC Management Tests', () => {
	...
	describe('Data Fetching - Audit Logs', () => {
		...
		it('should fetch audit logs with date filters', async () => {
			...
			expect(API.RBAC.getAuditLog).toHaveBeenCalledWith(expect.objectContaining({ startDate, endDate }))
		})
	...
	describe('Data Fetching - Users', () => {
		...
		it('should fetch users with role filter', async () => {
			...
			expect(API.RBAC.getUsersWithRoles).toHaveBeenCalledWith(expect.objectContaining({ roleFilter: 1000 }))
		})
		...
		it('should fetch users with search term', async () => {
			...
			expect(API.RBAC.getUsersWithRoles).toHaveBeenCalledWith(expect.objectContaining({ search: 'john' }))
		})
	})
	...
	describe('Edge Cases', () => {
		...
		it('should handle concurrent requests', async () => {
			...
			await act(async () => {
				...
				await Promise.all([result.current.fetchAuditLog(), result.current.fetchUsers()])
			})
