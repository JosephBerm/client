...
import { notificationService, formatDate, API, usePermissions, getRoleDisplayName } from '@_shared'
...
export default function AccountsDataGrid() {
	...
	const [deleteModal, setDeleteModal] = useState<{ isOpen: boolean; account: AccountInfo | null }>({
		isOpen: false,
		account: null,
	})
	...
	const [passwordModal, setPasswordModal] = useState<{ isOpen: boolean; account: AccountInfo | null }>({
		isOpen: false,
		account: null,
	})

	// Custom fetcher for RichDataGrid
	const fetcher = useCallback(async (filter: RichSearchFilter): Promise<RichPagedResult<AccountInfo>> => {
		const response = await API.Accounts.richSearch(filter)

		if (response.data?.payload) {
			return response.data.payload as unknown as RichPagedResult<AccountInfo>
		}
	...
	const handleRoleChange = async () => {
		...
		try {
			const { data } = await API.Accounts.updateRole(roleModal.account.id, roleModal.selectedRole)

			if (data.statusCode === 200) {
				const roleLabel = getRoleDisplayName(roleModal.selectedRole)
	...
	const openDeleteModal = (account: AccountInfo) => {
	...
	const openRoleModal = (account: AccountInfo) => {
		setRoleModal({
			isOpen: true,
			account,
			selectedRole: account.roleLevel,
		})
	}
	...
	const updateSelectedRole = (role: number | null) => {
		setRoleModal((prev) => ({ ...prev, selectedRole: role }))
	}

	const openPasswordModal = (account: AccountInfo) => {
	...
	const columnHelper = createRichColumnHelper<AccountInfo>()

	const columns: RichColumnDef<AccountInfo, unknown>[] = [
		...
		columnHelper.accessor('roleLevel', {
			header: 'Role',
			filterType: FilterType.Select,
			faceted: true,
			cell: ({ row }) => <RoleBadge role={row.original.roleLevel} />,
		}),
			...
			<RichDataGrid<AccountInfo>
				...
				bulkActions={
					isAdmin
						? ([
								...
								onAction: async (rows: AccountInfo[]) => {
										const headers = 'ID,Username,Email,Role,Status,Created\n'
										const csv = rows
											.map(
												(r) =>
													`${r.id},"${r.username ?? ''}","${r.email ?? ''}",${r.roleLevel},${
														r.status
								...
								confirmMessage: (count) =>
									`Are you sure you want to deactivate ${count} account(s)?`,
								onAction: async (rows: AccountInfo[]) => {
										const activeAccounts = rows.filter((r) => r.status === AccountStatus.Active)
										const promises = activeAccounts.map((r) =>
											API.Accounts.changeStatus(r.id, AccountStatus.Suspended)
										)
						  ...
						  ] satisfies BulkAction<AccountInfo>[])
						: undefined
				}
				...
				emptyState={
					<div className='flex flex-col items-center gap-3 py-12'>
						...
						<p className='text-sm text-base-content/40'>User accounts will appear here.</p>
					</div>
				}
