...
import { productPricingSchema, type ProductPricingFormData } from '@_core/validation'
 ...
 * ```
 */
export function useQuotePricing(quote: Quote | null, onRefresh?: () => Promise<void>): UseQuotePricingReturn {
	...
	const { submit, isSubmitting } = useFormSubmit(
		async (data: ProductPricingFormData) => {
			...
			return API.Quotes.updateProductPricing(quoteId, data.productId, data.vendorCost, data.customerPrice)
		},
		{
	 ...
	 */
	const validatePricing = useCallback((data: ProductPricingFormData): { valid: boolean; errors: string[] } => {
	 ...
	 */
	const canSendToCustomer = useCallback((q: Quote | null): boolean => {
