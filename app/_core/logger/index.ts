 ...
 * Maps old logger API to new logger system.
 * 
 * **DEPRECATED:** Use the new `logger` API instead.
 * This is provided for backward compatibility only.
 * 
 * **Migration Guide:**
 * ```typescript
 * // Old API
 * logger.log('message', data)        → logger.info('message', { data })
 * logger.error('message', error)     → logger.error('message', { error })
 * logger.warn('message', data)       → logger.warn('message', { data })
 * logger.group('label')              → logger.group('label')
 * logger.groupEnd()                  → logger.groupEnd()
 * logger.performance('label', start) → const timer = logger.startTimer('label'); timer()
 * ```
 * 
 * @deprecated Use the new logger API
 */
export const legacyLogger = {
	/**
	 * @deprecated Use logger.info() or logger.debug()
	 */
	log: (message: string, data?: any): void => {
		if (data !== undefined) {
			logger.info(message, typeof data === 'object' ? data : { data })
		} else {
			logger.info(message)
		}
	},
	...
	/**
	 * @deprecated Use logger.error() with structured metadata
	 */
	error: (message: string, error?: any): void => {
		if (error !== undefined) {
			logger.error(message, typeof error === 'object' ? { error } : { data: error })
		} else {
	...
	/**
	 * @deprecated Use logger.warn() with structured metadata
	 */
	warn: (message: string, data?: any): void => {
		if (data !== undefined) {
			logger.warn(message, typeof data === 'object' ? data : { data })
		} else {
			logger.warn(message)
		}
	},
	...
	/**
	 * @deprecated Use logger.groupEnd()
	 */
	groupEnd: (): void => {
		logger.groupEnd()
	},
	...
	/**
	 * @deprecated Use logger.startTimer() instead
	 * 
	 * @example
	 * ```typescript
	 * // Old way
