...
const LABEL_CLASS =
	'text-xs sm:text-xs font-semibold uppercase tracking-wide text-base-content/60 flex items-center gap-1.5'
...
export default function AccountOverview() {
	...
	const fullName =
		user.name && typeof user.name === 'object'
			? [user.name.first, user.name.middle, user.name.last].filter(Boolean).join(' ') ||
				user.username ||
				'MedSource User'
	...
	return (
		<div className='space-y-4 sm:space-y-6 w-full min-w-0'>
			...
			<Card
				...
				<div className='flex flex-col gap-5 sm:gap-7 md:gap-8 min-w-0'>
					...
					<div className='flex flex-col gap-4 sm:gap-4 min-w-0'>
						...
						<div className='flex items-start sm:items-center gap-2 min-w-0'>
							...
							<p
								className='text-sm sm:text-base text-base-content/70 truncate'
								title={customerName}>
							...
							<div className='space-y-3 pl-6'>
								...
								{formattedDateOfBirth && (
									<div className='space-y-1'>
										...
										<p className={VALUE_CLASS}>{formattedDateOfBirth}</p>
									</div>
								)}
							</div>
						</div>
						...
						<div className='min-w-0 space-y-4 sm:col-span-2 xl:col-span-1'>
							...
							<div className='space-y-3 pl-6'>
								<div className='space-y-1'>
									...
									<p className={VALUE_CLASS}>{memberSince}</p>
								</div>
								{hasActivity && (
									<div className='space-y-1'>
								...
								<div className='space-y-2.5 pl-6'>
									<div className='space-y-1'>
										{shippingAddress?.addressOne && (
											<p
												className={VALUE_CLASS}
												title={shippingAddress.addressOne}>
												{shippingAddress.addressOne}
											</p>
										)}
				...
				<div
					className='grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-6 py-2'
			...
			<Card
				...
				<div className='min-w-0'>
					...
					<h3
						className='text-lg sm:text-xl font-semibold text-base-content mb-4 truncate'
						title={customerName}>
						{customerName}
					</h3>
						...
						<div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4'>
							{user.customer.email && (
								<div className='flex items-center gap-2 min-w-0 w-full'>
									...
									<p
										className='text-sm sm:text-sm text-base-content/80 truncate min-w-0 flex-1'
							...
							{user.customer.users && user.customer.users.length > 0 && (
								<div className='flex items-center gap-2 min-w-0'>
									...
									<p className={VALUE_CLASS}>
										{user.customer.users.length}{' '}
										{user.customer.users.length === 1 ? 'user' : 'users'}
									</p>
								</div>
							)}
						</div>
