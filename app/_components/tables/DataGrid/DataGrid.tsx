...
export function DataGrid<TData>({
	...
	// NOTE: TanStack Table's getRowModel() is already memoized internally.
	// We don't need to memoize here - the table instance handles updates when
	// data, pagination, sorting, or filters change. Memoizing with [table] as
	// dependency fails because the table object reference is stable even when
	// its internal state changes. Calling getRowModel() directly ensures we
	// always get the latest computed rows.
	//
	// Dependencies that affect rows: normalizedData, pagination, sorting, columnFilters
	// These are all passed to useReactTable, which handles the memoization.
