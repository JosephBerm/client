...
export function DataGridPagination<TData>({
	...
	const canPreviousPage = isControlled ? pageIndex > 0 : table.getCanPreviousPage()
	const canNextPage = isControlled ? pageIndex < pageCount - 1 : table.getCanNextPage()
				...
				{enablePageSize && (
					<div className='flex items-center gap-2'>
						<label
							...
							className='text-sm text-base-content/70 whitespace-nowrap hidden sm:inline'>
							Rows:
						</label>
						<select
							id='page-size-select'
				...
				<div className='join'>
					{navigationButtons.map((button) => (
						<Button
							...
							variant='ghost'
							size='sm'
							className={classNames('join-item', TABLE_THEME_CLASSES.focusVisible)}
							type='button'
