			...
			<div className='flex-1'>
				...
				{error?.stack && (
					<details className='mt-2'>
						<summary className='cursor-pointer text-xs hover:underline'>Show stack trace</summary>
						<pre className='text-xs mt-2 p-2 bg-base-300 rounded overflow-auto max-h-40'>{error.stack}</pre>
					</details>
				)}
			</div>
		</div>
 ...
 */
export function RetryableErrorFallback({ error, onRetry }: { error?: Error; onRetry?: () => void }) {
	return (
		<div
			role='alert'
			className='alert alert-error'>
