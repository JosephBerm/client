...
export default function ContactInfo() {
	...
	return (
		<div
			ref={infoReveal.ref}
			className={classNames('transition-all duration-700 delay-200', {
			...
			<div className='mx-auto max-w-2xl rounded-2xl border border-base-300 bg-base-100 p-6 shadow-sm lg:max-w-5xl xl:max-w-6xl 2xl:max-w-7xl md:p-8'>
				<div className='relative grid gap-8 lg:grid-cols-2 lg:gap-12'>
					...
					<div className='space-y-4'>
						...
						<address
							className='not-italic space-y-4 text-sm text-base-content/70'
							itemProp='openingHours'>
							{businessHours.map((schedule, index) => (
								<div
									key={index}
									className='flex items-center justify-between gap-6'>
									<span className='font-medium text-base-content/90 min-w-[140px]'>
										{schedule.days}
									</span>
									{'closed' in schedule ? (
										...
										<span className='text-right'>
											{schedule.open} - {schedule.close} {schedule.timezone}
										</span>
									)}
								</div>
							))}
					...
					<div
						className='hidden lg:block absolute left-1/2 top-0 bottom-0 w-px bg-base-300 -translate-x-1/2'
						aria-hidden='true'
					/>
						...
						<ul className='space-y-2.5 text-sm text-base-content/70'>
							<li className='flex items-start gap-3'>
								...
								<span>
									...
									<span className='font-medium text-primary'>
										{COMPANY_CONTACT.responseTime.standard}
									</span>{' '}
							...
							<li className='flex items-start gap-3'>
								...
								<span>
									...
									<span className='font-medium text-primary'>
										{COMPANY_CONTACT.responseTime.emergency}
									</span>
								</span>
							</li>
							<li className='flex items-start gap-3'>
