...
export default function ImageGallery({
	...
	const validIndex = images && images.length > 0 ? Math.max(0, Math.min(currentIndex, images.length - 1)) : 0
				...
				<div className='flex gap-2 overflow-x-auto pb-2'>
					{images.map((image, index) => (
						<Button
							...
							variant='ghost'
							className={`relative shrink-0 overflow-hidden rounded-md border-2 transition-all p-0 h-auto ${
								index === validIndex
