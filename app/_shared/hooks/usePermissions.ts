...
import { DEFAULT_ROLE_THRESHOLDS, RBAC_QUERY_KEYS, RBAC_CACHE_CONFIG, RoleLevels } from '@_shared/constants'
...
export function usePermissions(): UsePermissionsReturn {
	...
	const { data: thresholdsData } = useFetchWithCache(RBAC_QUERY_KEYS.thresholds, () => API.RBAC.getThresholds(), {
	...
	const {
		...
		isLoading,
		refetch,
	} = useFetchWithCache(user?.id ? RBAC_QUERY_KEYS.permissions(user.id) : '', () => API.RBAC.getMyPermissions(), {
		enabled: !!user,
	...
	const isFulfillmentCoordinatorOrAbove =
