...
import { parseRequiredTimestamp, parseOptionalTimestamp } from '@_lib/dates'
 ...
 * 
 * **Backend Alignment:** Matches server/Entities/Quote.cs structure
 */
export default class Quote {
	/** Unique identifier (GUID, auto-generated) */
	id: string = ''
	
	/** Sequential quote number (auto-generated by database) */
	...
	/** Contact name (formatted, for display) */
	contactName: string = ''
	 ...
	 * Default: 30 days from creation (per business_flow.md).
	 */
	validUntil?: Date
	...
	/** Whether quote is archived (soft delete) */
	...
	// =========================================================================
	// QUOTE STATUS - Business Flow Section 1.5
	// =========================================================================
	
	/** Current quote status (Unread, Read, Approved, Rejected, Converted, Expired) */
	status: QuoteStatus = QuoteStatus.Unread
	
	/** Priority level for processing (Standard, High, Urgent) */
	priority: QuotePriority = QuotePriority.Standard
	
	// =========================================================================
	// SALES REP ASSIGNMENT - Business Flow Section 2.2
	// =========================================================================
	
	/** ID of the sales rep assigned to this quote */
	...
	constructor(param?: Partial<Quote>) {
		if (param) {
			...
			// Parse creation date from string if provided (from API response)
			// For new quotes being submitted, createdAt is set by the server
			if (param.createdAt !== undefined && param.createdAt !== null) {
				this.createdAt = parseRequiredTimestamp(param.createdAt, 'Quote', 'createdAt')
			}
			// Otherwise keep the default (new Date()) for new quote submissions
