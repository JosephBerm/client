 ...
 * - Timestamp tracking
 * - Referral tracking for sales rep attribution (business_flow.md Section 2.2)
 * 
 * **Quote Status Flow:**
 ...
 * @example
 * ```typescript
 * // Create a quote request with referral
 ...
 *   referredBy: 'salesrep@medsourcepro.com',
 *   referralSource: 'url',
 *   products: [
 *     new CartProduct({
 *       product: product1,
 ...
 * ```
 * 
 * @see business_flow.md Section 1 - Quote Request Submission
...
export default class Quote {
	/** Unique identifier (GUID, auto-generated by backend for new quotes) */
	id?: string
	...
	/** Quote validity expiration date (default: 30 days from creation) */
	validUntil?: Date
	
	/** Current quote status (Unread, Read, Approved, Rejected, Converted) */
	status: QuoteStatus = QuoteStatus.Unread
	
	/** Quote processing priority (Standard, High, Urgent) */
	priority: QuotePriority = QuotePriority.Standard
	
	// =========================================================================
	// REFERRAL TRACKING - Business Flow Section 2.2
	// =========================================================================
	
	/**
	 * Email or identifier of the sales rep who referred this customer.
	 * Captured from URL param (?ref=), QR code, or manual input.
	 * 
	 * @see business_flow.md Section 2.2 - Referral-Based Assignment
	 */
	referredBy?: string
	...
	/**
	 * Source of the referral: 'url', 'qr', or 'manual'.
	 * Helps track which referral methods are most effective.
	 */
	referralSource?: 'url' | 'qr' | 'manual'
	
	/**
	 * ID of the assigned sales rep.
	 * Set automatically based on referral or customer relationship.
	 */
	assignedSalesRepId?: string
	
	/**
	 * Timestamp when quote was assigned to sales rep.
	 */
	assignedAt?: Date
	...
	constructor(param?: Partial<Quote>) {
		if (param) {
			...
			if (param.validUntil) {
				this.validUntil = parseDateSafe(param.validUntil) ?? undefined
			}
			if (param.assignedAt) {
	...
	/**
	 * Check if the quote has expired based on validUntil date.
	 */
	isExpired(): boolean {
	...
	/**
	 * Check if the quote has a referral attributed.
	 */
	hasReferral(): boolean {
		return Boolean(this.referredBy)
	}
	
	/**
	 * Check if the quote is assigned to a sales rep.
	 */
	isAssigned(): boolean {
		return Boolean(this.assignedSalesRepId)
	}
}
